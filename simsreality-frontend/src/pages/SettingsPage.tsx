...
const selectClassName =
  "px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-sm text-white focus:outline-none focus:border-[#00d4ff]/50 font-['JetBrains_Mono',monospace]";
...
function SettingsPage() {
  ...
  return (
    <main className="min-h-full overflow-auto text-white font-['Rajdhani',sans-serif]">
      <div className="max-w-2xl mx-auto p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-wide">설정</h1>
          </div>
          <motion.button
            type="button"
            ...
            whileTap={{ scale: 0.97 }}
            onClick={handleSave}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold tracking-wider text-sm transition-colors"
            style={{ background: showSaved ? '#00ff88' : '#00d4ff', color: '#020b18' }}
          >
        ...
        <SettingsSection icon={User} eyebrow="PROFILE" title="프로필">
          {isProfileLoading ? (
            <p className="px-6 py-3 text-xs text-white/50 font-['JetBrains_Mono',monospace]">
              내 정보를 불러오는 중...
            </p>
          ) : null}
          {profileErrorMessage ? (
            <p className="px-6 py-3 text-xs text-[#f26d6d] font-['JetBrains_Mono',monospace]">
              {profileErrorMessage}
            </p>
          ) : null}
          <ProfileFields
        ...
        <SettingsSection icon={Globe} eyebrow="DISPLAY" title="화면 설정">
          ...
          <SettingsRow label="언어">
            ...
            >
              <option value="ko" className="bg-[#0a1628]">
                한국어
              </option>
              <option value="en" className="bg-[#0a1628]">
                English
              </option>
            </select>
          </SettingsRow>
          <SettingsRow label="시간대">
            ...
            >
              <option value="Asia/Seoul" className="bg-[#0a1628]">
                Asia/Seoul (KST)
              </option>
              <option value="UTC" className="bg-[#0a1628]">
                UTC
              </option>
              <option value="America/New_York" className="bg-[#0a1628]">
                America/New_York (EST)
              </option>
            </select>
          </SettingsRow>
          <SettingsRow label="다크 모드 아이콘">
            <Moon className="w-5 h-5 text-white/30" />
          </SettingsRow>
        </SettingsSection>
      </div>
    </main>
